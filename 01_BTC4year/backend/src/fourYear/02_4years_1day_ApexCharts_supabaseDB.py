...
<body>
    ...
    <script>
        ...
        series.forEach(function(s, idx) {{
            ...
            var labelText = 'Cycle ' + (idx + 1) + ' : ' + s.startDate;
            
            var card = document.createElement('div');
            card.className = 'stat-card ' + colorClass;
            card.dataset.index = idx;
            card.innerHTML = '<div class="stat-label ' + colorClass + '">' + labelText + '</div>' +
                           '<div class="stat-value">' +
                           '<span class="stat-min">최저 ' + minVal.toFixed(2) + '%</span>' +
                           '<span class="stat-days">' + s.dayCount + ' days</span>' +
                           '</div>';
